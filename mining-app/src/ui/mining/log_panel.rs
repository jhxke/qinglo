...
pub fn view_log_panel(state: &UiState) -> Element<'_, Message> {
    ...
    // 右侧操作组：清日志（左箭头内含 ×）+ 关闭，纯图标风格统一协调
    let clear_btn = icon_action_btn(IconKind::Clear, theme::warning(), Message::ClearLogs);
    let close_btn = icon_action_btn(IconKind::Close, theme::text_strong(), Message::ToggleLogPanel);
...
/// 日志面板右上操作图标按钮（清日志 / 关闭 共用）：
/// 26×26 透明底，hover 时图标按语义色提亮，纯图标风格与关闭按钮统一协调。
fn icon_action_btn(
    icon_kind: IconKind,
    hover_color: Color,
    msg: Message,
) -> Element<'static, Message> {
    let icon = icons::view_icon_with_stroke(icon_kind, theme::text_weak(), 11.0, 1.4);
        ...
        .style(move |_t, status| {
            ...
            match status {
                ...
                iced::widget::button::Status::Pressed => {
                    s.background = Some(Color { r: 1.0, g: 1.0, b: 1.0, a: 28.0 / 255.0 }.into());
                    s.text_color = hover_color;
                }
                _ => {}
            }
