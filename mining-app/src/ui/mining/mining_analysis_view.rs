...
// ===== iced_aw 组件导入 =====
// 引入 iced_aw::Card，替换部分手搓组件，提升 UI 质感。
...
fn view_sidebar(state: &UiState) -> Element<'_, Message> {
    ...
    let content: Element<'_, Message> = match editor.active_left_panel {
        LeftPanelTab::Models => super::models_panel::view_models_panel(state),
        LeftPanelTab::Operators => super::operator_panel::view_operator_panel(state),
    };
...
// 建模列表视图已拆分至 `super::models_panel` 模块。
...
// 算子面板视图已拆分至 `super::operator_panel` 模块。
