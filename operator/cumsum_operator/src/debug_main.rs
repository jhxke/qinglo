use cumsum_operator::{execute_operator, release_port_data};
...
/// 以 DataFrameArray 输入运行一次累加算子，返回输出 PortData
fn run_cumsum(input_dfs: Vec<DataFrame>, params_json: &str) -> Option<PortData> {
...
fn main() {
    let df_a = build_stock_df(10.0, 6);
    let df_b = build_stock_df(50.0, 6);
    println!(
        "=== 输入 DataFrameArray ===\n  A: 行数={}, 列名={:?}\n  B: 行数={}, 列名={:?}",
        df_a.row_count,
    ...
    // ---------- 1. 单列就地累加（result_column 缺省 → 覆盖源列） ----------
    println!("\n########## 测试 1：单列就地累加 volume（result_column 缺省）##########");
    let json = r#"{ "columns": "volume" }"#;
    if let Some(pd) = run_cumsum(vec![df_a.clone(), df_b.clone()], json) {
        match pd {
    ...
    // ---------- 2. 单列累加到新列名（result_column=cum_close，源列保留） ----------
    println!("\n########## 测试 2：累加 close 到新列 cum_close（源列保留）##########");
    let json = r#"{ "columns": "close", "result_column": "cum_close" }"#;
    if let Some(pd) = run_cumsum(vec![df_a.clone()], json) {
        match pd {
            PortData::DataFrameArray(dfs) => {
                println!("输出 DataFrameArray ({} 个，应为 1)，列名={:?}",
                    dfs.len(),
    ...
    // ---------- 3. 单个 DataFrame 输入（兼容性） ----------
    println!("\n########## 兼容性：单个 DataFrame 输入 ##########");
    if let Some(pd) = run_cumsum(vec![df_a.clone()], r#"{ "columns": "volume" }"#) {
        match pd {
            PortData::DataFrameArray(dfs) => {
    ...
    // ---------- 4. 含不存在列与字符串列（应跳过） ----------
    println!("\n########## 测试跳过：code(字符串) / nope(不存在) ##########");
    if let Some(pd) = run_cumsum(vec![df_a], r#"{ "columns": "nope", "result_column": "cum_nope" }"#) {
        match pd {
            PortData::DataFrameArray(dfs) => {
                print_df(&dfs[0], 6);
            }
    ...
    // ---------- 5. 空参数（应返回错误码 -6） ----------
    println!("\n########## 空参数测试（应返回 -6）##########");
    let _ = run_cumsum(vec![df_b.clone()], r#"{ "columns": "" }"#);

    // ---------- 6. 多列含逗号（应返回错误码 -7） ----------
    println!("\n########## 多列含逗号测试（应返回 -7）##########");
    let _ = run_cumsum(vec![df_b], r#"{ "columns": "close,volume" }"#);
}
