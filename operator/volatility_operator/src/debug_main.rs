...
use std::ffi::CString;
use std::ptr;
use volatility_operator::{execute_operator, release_port_data};
...
/// 以 DataFrameArray 输入运行一次波动率算子，返回输出 PortData
fn run_volatility(input_dfs: Vec<DataFrame>, params_json: &str) -> Option<PortData> {
...
fn main() {
    ...
    // ---------- 1. 默认 n=5，结果列自动取 volatility_5 ----------
    println!("\n########## 测试 1：默认 n=5，结果列自动取 volatility_5 ##########");
    let json = r#"{ "source_column": "close" }"#;
    if let Some(pd) = run_volatility(vec![df_a.clone(), df_b.clone()], json) {
        match pd {
    ...
    // ---------- 2. 自定义 n 与结果列名（源列保留） ----------
    println!("\n########## 测试 2：n=3, result_column=vol_3（源列保留）##########");
    let json = r#"{ "n": "3", "result_column": "vol_3", "source_column": "close" }"#;
    if let Some(pd) = run_volatility(vec![df_a.clone()], json) {
        match pd {
            PortData::DataFrameArray(dfs) => {
                println!(
    ...
    println!("\n########## 测试 3：Int64 源列 volume，n=3，结果提升为 Float64 ##########");
    let json = r#"{ "n": "3", "source_column": "volume", "result_column": "vol_vol_3" }"#;
    if let Some(pd) = run_volatility(vec![df_a.clone()], json) {
        match pd {
            PortData::DataFrameArray(dfs) => {
                let col = dfs[0].column("vol_vol_3").unwrap();
                println!("vol_vol_3 类型: {:?}", col.data_type);
                print_df(&dfs[0], 8);
            }
            other => println!("意外输出类型: {}", other.type_name()),
        }
    ...
    println!("\n########## 兼容性：单个 DataFrame 输入 ##########");
    if let Some(pd) = run_volatility(vec![df_a.clone()], r#"{ "n": "2" }"#) {
        match pd {
            PortData::DataFrameArray(dfs) => {
    ...
    if let Some(pd) = run_volatility(vec![df_a.clone()], r#"{ "n": "5", "source_column": "nope" }"#) {
        match pd {
            PortData::DataFrameArray(dfs) => {
                print_df(&dfs[0], 8);
            }
    ...
    // ---------- 6. 非法 n（应返回错误码 -6） ----------
    println!("\n########## 非法 n 测试（应返回 -6）##########");
    let _ = run_volatility(vec![df_a.clone()], r#"{ "n": "0" }"#);
    let _ = run_volatility(vec![df_a.clone()], r#"{ "n": "abc" }"#);

    // ---------- 7. n 大于行数（结果全空） ----------
    println!("\n########## 测试 7：n=10 大于行数 8（结果全空）##########");
    if let Some(pd) = run_volatility(vec![df_b.clone()], r#"{ "n": "10", "result_column": "vol_10" }"#) {
        match pd {
            PortData::DataFrameArray(dfs) => {
                print_df(&dfs[0], 8);
            }
    ...
    if let Some(pd) = run_volatility(vec![df_b], r#"{ "n": "2", "source_column": "code" }"#) {
        match pd {
            PortData::DataFrameArray(dfs) => {
                print_df(&dfs[0], 8);
            }
