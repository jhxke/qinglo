use rsi_operator::{execute_operator, release_port_data};
...
/// 以 DataFrameArray 输入运行一次 RSI 算子，返回输出 PortData
///
...
fn run_rsi(input_dfs: Vec<DataFrame>, params_json: &str) -> Option<PortData> {
...
fn main() {
    ...
    if let Some(pd) = run_rsi(vec![df_a.clone(), df_b.clone()], rsi_json) {
        print_output("RSI", &pd, 8);
    }

    // ---------- 2. 兼容性：单个 DataFrame 输入 ----------
    println!("\n########## 兼容性测试：单个 DataFrame 输入（应输出单元素 DataFrameArray）##########");
    if let Some(pd) = run_rsi(vec![df_a.clone()], rsi_json) {
        print_output("Single-DF RSI", &pd, 5);
    }
    ...
    let sc_json = r#"{ "rsi_period": "14", "source_column": "price" }"#;
    if let Some(pd) = run_rsi(vec![df_price], sc_json) {
        print_output("RSI-SOURCE-COL", &pd, 5);
    }

    // ---------- 4. 空参数：原样返回输入 ----------
    println!("\n########## 空参数测试：原样返回输入 ##########");
    let empty_json = r#"{}"#;
    if let Some(pd) = run_rsi(vec![df_a, df_b], empty_json) {
