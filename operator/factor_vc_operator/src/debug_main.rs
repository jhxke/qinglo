use factor_vc_operator::{execute_operator, release_port_data};
use operator_runtime::{DataFrame, PortData};
use operator_runtime::c_abi::{portdata_to_c, portdata_from_c, CPortData, CPortValue, TYPE_NULL};
use std::ffi::CString;
use std::ptr;
...
fn run_factor_vc(input_dfs: Vec<DataFrame>, params_json: &str) -> Option<PortData> {
...
fn main() {
    ...
    // 1. 默认 N=20（df_a 15 行不足；df_b 25 行，i>=21 有效）
    println!("\n########## 测试默认 N=20 ##########");
    let default_json = r#"{}"#;
    if let Some(pd) = run_factor_vc(vec![df_a.clone(), df_b.clone()], default_json) {
        print_output("默认", &pd, 5);
    }

    // 2. N=3（两个 DataFrame 都能产出有效因子）
    println!("\n########## 测试 N=3 ##########");
    let n3_json = r#"{"n":"3"}"#;
    if let Some(pd) = run_factor_vc(vec![df_a.clone(), df_b.clone()], n3_json) {
        print_output("N=3", &pd, 10);
    }
    ...
    if let Some(pd) = run_factor_vc(vec![df_a.clone()], custom_json) {
        print_output("自定义", &pd, 8);
    }

    // 4. 兼容性：单个 DataFrame 输入
    println!("\n########## 兼容性：单个 DataFrame 输入 ##########");
    if let Some(pd) = run_factor_vc(vec![df_b.clone()], n3_json) {
        print_output("单DF", &pd, 8);
    }
}
