use price_volume_factor_operator::{execute_operator, release_port_data};
...
fn run_pv_factor(input_dfs: Vec<DataFrame>, params_json: &str) -> Option<PortData> {
...
fn main() {
    ...
    // 1. 默认 N=20（df_a 只有 10 行，全空；df_b 前 19 行空，最后 1 行有效）
    println!("\n########## 测试默认 N=20 ##########");
    let default_json = r#"{}"#;
    if let Some(pd) = run_pv_factor(vec![df_a.clone(), df_b.clone()], default_json) {
    ...
    println!("\n########## 测试 N=3 ##########");
    let n3_json = r#"{"n":"3"}"#;
    if let Some(pd) = run_pv_factor(vec![df_a.clone(), df_b.clone()], n3_json) {
    ...
    println!("\n########## 测试自定义列名 ##########");
    let custom_json =
        r#"{"n":"3","price_column":"close","volume_column":"volume","result_column":"my_f1"}"#;
    if let Some(pd) = run_pv_factor(vec![df_a.clone()], custom_json) {
        print_output("自定义", &pd, 5);
    }

    // 4. 兼容性：单个 DataFrame 输入
    println!("\n########## 兼容性：单个 DataFrame 输入 ##########");
    if let Some(pd) = run_pv_factor(vec![df_b.clone()], n3_json) {
