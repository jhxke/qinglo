use macd_operator::{execute_operator, release_port_data};
...
/// 以 DataFrameArray 输入运行一次 MACD 算子，返回输出 PortData
///
...
fn run_macd(input_dfs: Vec<DataFrame>, params_json: &str) -> Option<PortData> {
...
fn main() {
    ...
    if let Some(pd) = run_macd(vec![df_a.clone(), df_b.clone()], macd_json) {
        print_output("MACD", &pd, 8);
    }

    // ---------- 2. 仅配置部分参数（回退默认） ----------
    println!("\n########## 测试部分参数回退默认 (仅 macd_fast=12) ##########");
    let partial_json = r#"{ "macd_fast": "12" }"#;
    if let Some(pd) = run_macd(vec![df_a.clone(), df_b.clone()], partial_json) {
        print_output("MACD-PARTIAL", &pd, 8);
    }

    // ---------- 3. 兼容性：单个 DataFrame 输入 ----------
    println!("\n########## 兼容性测试：单个 DataFrame 输入（应输出单元素 DataFrameArray）##########");
    if let Some(pd) = run_macd(vec![df_a.clone()], macd_json) {
        print_output("Single-DF MACD", &pd, 5);
    }

    // ---------- 4. 自定义源列名（source_column=price，非标准 close） ----------
    ...
    let sc_json = r#"{ "macd_fast": "12", "macd_slow": "26", "macd_signal": "9", "source_column": "price" }"#;
    if let Some(pd) = run_macd(vec![df_price], sc_json) {
        print_output("MACD-SOURCE-COL", &pd, 5);
    }

    // ---------- 5. 空参数：原样返回输入 ----------
    println!("\n########## 空参数测试：原样返回输入 ##########");
    let empty_json = r#"{}"#;
    if let Some(pd) = run_macd(vec![df_a, df_b], empty_json) {
