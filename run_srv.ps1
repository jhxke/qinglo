...
# which would corrupt literal Chinese. Operators deploy to
# lib\operator\<group>\<operator>\; the shared operator_runtime.dll deploys to a
# single lib\public\ (the server registers it as a DLL search dir).
...
#    each run removes stale entries from any previous layout (e.g. the old
#    lib\<group>\<operator>\ structure) that would otherwise surface as
#    duplicate/ungrouped operators in the frontend.
...
# 2a. Shared runtime: a single operator_runtime.dll under lib\public\ (instead of one
#     copy per operator dir). The server adds this directory to the DLL search path
#     (SetDllDirectory) before preloading operators.
$publicDir = Join-Path $libRoot "public"
New-Item -ItemType Directory -Path $publicDir -Force | Out-Null
if (Test-Path $runtimeDllSrc) {
    Copy-Item $runtimeDllSrc $publicDir
    Write-Host "operator_runtime.dll copied to lib\public\" -ForegroundColor Cyan
}

# Group operators by Group (preserving first-seen order), deploy to
# lib\operator\<group>\<operator>\
$grouped = [ordered]@{}
foreach ($op in $operators) {
...
$operatorRoot = Join-Path $libRoot "operator"
foreach ($groupEntry in $grouped.GetEnumerator()) {
    ...
    foreach ($op in $groupEntry.Value) {
        $destDir = Join-Path $operatorRoot "$($groupEntry.Key)\$($op.Dir)"
        Write-Host "Copying $($op.Name).dll -> lib\operator\$($groupEntry.Key)\$($op.Dir)\" -ForegroundColor Yellow
        ...
        # operator_runtime.dll is NOT copied here: the shared copy in lib\public\
        # is resolved via the server's extra DLL search directory.
